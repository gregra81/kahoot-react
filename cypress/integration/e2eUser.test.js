...
describe('kahoot', () => {
  it('Gets to the lobby and displays pin and nick', () => {
    ...
    cy.request('http://localhost:3030/host-game?pin=0').as('game-creation');
  ...
  it('Starts the game and checks that there is a question and answer', () => {
    cy.request('http://localhost:3030/start-game?questionNumber=0').as('game-started');
  ...
  it('Checks that you can click only one answer, only one time.', () => {
    cy.request('http://localhost:3030/next-question?questionNumber=1').as('second-question');
  ...
  it('Checks that the amount of answers rendered is dynamic', () => {
    cy.request('http://localhost:3030/next-question?questionNumber=2').as('final-question');
  ...
  it('Checks that you are redirected to the podium after the final answer', () => {
    cy.request('http://localhost:3030/podium').as('podium');
